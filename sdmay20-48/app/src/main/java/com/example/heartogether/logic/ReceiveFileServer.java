...
public class ReceiveFileServer extends AbstractReceiver {
    private static final int SERVER_PORT = 4445;
    private Context mContext;
    private ServerSocket serverSocket;

    public ReceiveFileServer(Context context){
        mContext = context;
    }
    ...
    protected void onProgressUpdate(Message... values) {
        ...
        if (type== Message.AUDIO_MESSAGE){
            values[0].saveByteArrayToFile(mContext);
        }

        new SendFileServer(mContext, false).executeOnExecutor(THREAD_POOL_EXECUTOR, values);
    }
}
